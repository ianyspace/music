...
/**
 * Every key in the store, without the records behind them.
 *
 * `getAll()` hands back every blob, so on a phone it is an expensive call that
 * loads the whole cache into memory; `getAllKeys()` is a key-only walk. Any
 * caller that only needs to know *what* is cached (a sweep, a count) wants this
 * one.
 */
export const readAllKeys = async function () {
    try {
