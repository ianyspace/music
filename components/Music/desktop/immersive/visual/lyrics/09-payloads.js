/**
 * 舞台歌词 payload 归一化 / 图层拆分。
 *
 * 移植自上游 Mineradio public/js/modules/02-visual/09-lyrics-payloads.js
 * 上游许可证 GNU GPL v3 (用户已授权原样复制)。
 *
 * 为什么是原样搬运: role 白名单、alpha / scale / lineOffset 的默认分档与
 * clamp 区间、以及 active / row-base / context 三层 payload 各自的 override
 * 数值都是和实际渲染对齐过的, 单独改一个会破坏三层之间的互补关系, 因此逐行
 * 保留, 包括 activeLayer 与 contextLayer 互斥 alpha 的处理方式。
 *
 * 相对上游的改动仅限:
 *   - 顶层 function 改为 ESM export;
 *   - `fx` `normalizeLyricTranslationText` 改从 ./runtime.js 取;
 *   - `clampRange` 改用本仓库 visual/presetData.js 的实现;
 *   - `lyricFontWeightValue` 从 ./05-fonts-texture.js 取,
 *     `normalizeLyricDisplayMode` `lyricContextOpacityValue`
 *     `lyricTranslationScaleValue` 从 ./08-display-modes.js 取。
 */

import { clampRange } from '../presetData.js';
import { fx, normalizeLyricTranslationText } from './runtime.js';
import { lyricFontWeightValue } from './05-fonts-texture.js';
import {
...
export function normalizeStageLyricText(text) {
    return String(text || '').replace(/\s+/g, ' ').trim();
}
export function normalizeStageLyricEntry(entry, fallbackRole) {
...
export function lyricEntryWeight(entry) {
...
export function cloneStageLyricEntryForLayer(entry, overrides) {
    entry = entry || {};
    var copy = {
        text: entry.text,
        role: entry.role,
...
export function activeStageLyricPayload(payload) {
...
export function rowBaseStageLyricPayload(payload) {
    payload = normalizeStageLyricPayload(payload);
...
export function contextStageLyricPayload(payload) {
    payload = normalizeStageLyricPayload(payload);
