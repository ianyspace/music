/**
 * 移植自上游 Mineradio public/js/modules/02-visual/03-lyrics-star-river.js。
 * 上游许可证 GNU GPL v3（用户已授权原样复制）。
 * 这里逐行保留歌词入口、星河与延迟释放逻辑及全部观感数值，仅做 ESM 适配、
 * THREE 惰性初始化，并按要求移除调试统计，避免重写或优化改变原有效果。
 */

import { SKULL_PRESET_INDEX, clampRange } from '../presetData.js';
import { THREE, fx, scene, uniforms } from './runtime.js';
import { ensureStageLyricLayoutState, stageLyrics } from './02-state-layout.js';
import { lyricThreeColor } from './07-palette-utils.js';
import {
...
// 上游 dotTexture 来自未纳入本次移植的 00-pointer-cover-particles.js；
// 按原函数的数值与逻辑在本文件惰性创建，避免 import 期访问尚未加载的 THREE。
var dotTexture = null;
...
export function createLyricsParticles() {
...
export function ensureLyricStarRiver() {
...
export function disposeLyricStarRiver() {
...
export function updateLyricStarRiver(dt) {
...
export function disposeOwnedLyricTexture(texture) {
...
export function disposeLyricMaterial(material) {
    if (!material) return;
...
export function flushLyricDisposeQueue() {
    lyricDisposeTimer = 0;
...
export function disposeLyricMesh(mesh) {
