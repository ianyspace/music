/**
 * 移植自上游 Mineradio public/js/modules/02-visual/12-lyrics-row-layers.js（1787 行）。
 * 上游许可证 GNU GPL v3（用户已授权原样复制）。
 *
 * 为什么原样搬运：本模块里每一个数字都是调过的观感参数 —— 行间距
 * (lyricTrackLineStepWorld / lyricTranslationLineStepWorld)、深度曲线
 * (0.055 - pow(abs, 1.06) * 0.145)、viewport 安全边距 (0.045 / 42~92px)、
 * 上下文行透明度曲线、分步构建的相位计数、纹理质量队列的容量与热度窗口，
 * 全部互相耦合。任何「顺手重构」都会改变歌词的排布与呼吸感，因此这里逐行
 * 保留上游实现，只做 IIFE 全局 -> ESM 的机械转换。
 *
 * 本次移植的三处有意改动：
 * 1. THREE 惰性化：上游顶层的 `new THREE.Vector3()`（lyricViewportFitLeft /
 *    lyricViewportFitRight）改为 null + 惰性创建，因为 ESM 在 import 期
 *    THREE 尚未 bindStage。函数体内使用 THREE 不受影响（import 绑定是活的）。
 * 2. 每帧上传预算退化：resetLyricRenderUploadFrameBudget /
 *    consumeLyricRenderUploadFrameBudget 保留函数签名与调用形状，但内部改为
 *    始终允许（remaining = Infinity / 返回 true）。这两个函数是桌面 Electron
 *    的显存上传治理，网页端没有对等机制；它们的边界安全性依赖调用点，直接删除
 *    会牵动 renderRevealCandidates 等大量调用点，故保留而退化。
 *    lyricQualityEffectiveBudgetBytes / lyricQualityEffectiveMaxRows 同理：
 *    保留池化显存预算的形状，去掉硬件档位分支。
 * 3. 丢弃调试导出：window.__mineradioLyricQualityStats /
 *    __mineradioLyricUploadBudgetStats 全部移除；requestIdleCallback /
 *    cancelIdleCallback 换成 runtime.js 的 requestIdle / cancelIdle。
 * shelf（书架）相关分支在此文件中不存在，无需处理。
 */
...
import {
    LYRIC_GLOW_BUILD_PHASES,
    LYRIC_READABILITY_BUILD_PHASES,
    beginLyricGlowTextureBuild,
    beginLyricReadabilityTextureBuild,
    compactLyricLineMaskTexture,
    finishLyricGlowTextureBuild,
    finishLyricReadabilityTextureBuild,
    lyricGlowRasterMetrics,
    lyricQualityMaxResidentRows,
    lyricQualityPoolBudgetBytes,
    lyricQualityTargetMetrics,
    lyricTextureClarityScale,
    makeLyricGlowTexture,
    makeLyricMask,
    makeLyricQualityTexture,
    stepLyricGlowTextureBuild,
    stepLyricReadabilityTextureBuild,
...
import {
    ...
    lyricEntryLineOffset,
    normalizeStageLyricPayload,
} from './09-payloads.js';
import { lyricRowGlowThreeColor, lyricThreeColor } from './07-palette-utils.js';
import { makeLyricShaderMaterial } from './11-lyrics-shaders.js';
import { disposeLyricMesh, disposeOwnedLyricTexture } from './03-star-river.js';
import { lyricVerticalFloatEnabled, stageLyricResidentDisplayedScrollOffset } from './14-stage-rendering.js';

// 上面几个颜色 / 数值工具（normalizeHexColor、rgbToHsl、hslToRgb、rgbCss、
// clamp01、layoutNumber、layoutInteger、normalizeLyricTextureClarity）按移植
// 约定统一 import，本文件的这一段并未直接用到，保留以便兄弟模块对齐。

// ============================================================
// 行层几何: 每行在世界坐标里的中心 Y / 深度 / 缩放
// ============================================================

export function lyricLineCenterWorldY(mask, entry, lineIndex, worldH) {
    mask = mask || {};
    entry = entry || {};
...
export function lyricRowVirtualIndex(entry, fallbackIndex) {
    entry = entry || {};
...
export function lyricLayerVirtualIndex(entry, fallbackIndex, activeLine, usesTrack) {
    entry = entry || {};
    if (!usesTrack) {
...
export function lyricTrackLineStepWorld(mask, worldH) {
    mask = mask || {};
    var h = Math.max(1, Number(mask.height) || 384);
...
export function lyricTranslationLineStepWorld(mask, worldH) {
    mask = mask || {};
    var h = Math.max(1, Number(mask.height) || 384);
    var lineHeight = Number(mask.lineHeight) || Number(mask.fontSize) || 128;
    var step = worldH * (lineHeight / h);
...
export function lyricTranslationAnchoredY(entry, fallbackIndex, activeLine, lineStepWorld, translationLineStepWorld, scrollOffset, rowDrift, currentTranslation, usesTrack) {
    entry = entry || {};
    if (!usesTrack) {
...
export function lyricLineAllowedForDisplayMode(lineIndex, targetLineIndex, mode) {
...
export function lyricNearestPrimaryLineIndexForVirtual(virtualIndex, fallbackLineIndex) {
...
export function lyricRowVisualDelta(entry, index, activeLine) {
    entry = entry || {};
...
export function lyricRowDepthZ(entry, index, activeLine) {
...
export function lyricRowDepthScale(entry, index, activeLine) {
...
export function makeLyricBackfaceReadableMaterial(opts) {
    opts = opts || {};
...
export function setLyricTextureMaterialOpacity(mat, value) {
...
export function getLyricTextureMaterialOpacity(mat) {
...
export function setLyricTextureMaterialColor(mat, color) {
    if (!mat || !color) return;
...
// 可读性描边用的三个常量色。上游在 IIFE 顶层按需 new THREE.Color()，
// 这里保持模块级变量，但第一次用到时才惰性创建（THREE 运行时才可用）。
export var lyricReadabilityLightColor = null;
export var lyricReadabilityDarkColor = null;
export var lyricReadabilityMixColor = null;

export function lyricBackgroundAdaptStrengthValue() {
...
export function lyricBackdropAdaptActive() {
    return (!fx || fx.lyricBackdropAdapt !== false) && lyricBackgroundAdaptStrengthValue() > 0.001;
}

export function lyricReadabilityColorForBrightBackdrop(strength) {
...
// ============================================================
// 单行 mask / 尺寸 / viewport 自适应
// ============================================================

export function makeLyricLineMask(entry, baseMask, asActive) {
    entry = entry || {};
...
export function lyricTranslationMeshScale(entry) {
...
export function lyricRowLogicalWorldWidth(mask, baseWorldW) {
    mask = mask || {};
...
export function lyricViewportSafeMarginPx(viewportWidth) {
    return clampRange(Math.max(1, Number(viewportWidth) || 1) * 0.045, 42, 92);
}

export function lyricViewportFitRatio(viewportWidth, centerX, projectedUnitTextWidth, intendedScale, marginPx) {
...
// 上游在 IIFE 顶层 `new THREE.Vector3()`; ESM 下 THREE 运行时才可用, 改为惰性创建。
export var lyricViewportFitLeft = null;
export var lyricViewportFitRight = null;

function ensureLyricViewportFitVectors() {
    if (!lyricViewportFitLeft) lyricViewportFitLeft = new THREE.Vector3();
    if (!lyricViewportFitRight) lyricViewportFitRight = new THREE.Vector3();
}

export function lyricRowLiveViewportScale(row, intendedScale) {
    if (!row || !row.mesh || !row.lineMask || typeof camera === 'undefined' || !camera) return 1;
    ensureLyricViewportFitVectors();
    if (!lyricViewportFitLeft || !lyricViewportFitRight) return 1;
...
export function makeLyricRowGlowMesh(row, pal, worldW, preparedGlowTexture) {
...
export function beginLyricRowLayerGroupBuild(payload, mask, worldW, worldH, pal, motionProfile) {
...
export function beginLyricRowLayerBuildEntry(state) {
...
export function appendLyricRowReadabilityLayer(state) {
...
export function finishLyricRowLayerBuildEntry(state) {
    var row = state && state.pendingRow;
...
export function appendLyricRowLayerBuildPhase(state) {
    if (!state || state.done) return false;
...
export function appendLyricRowLayerBuildEntry(state) {
    if (!state || state.done) return false;
...
export function stepLyricRowLayerGroupBuild(state, maxPhases, budgetMs) {
...
export function finishLyricRowLayerGroupBuild(state) {
...
export function releaseLyricRowLayerBuildCanvas(build) {
    if (!build) return;
...
export function cancelLyricRowLayerGroupBuild(state) {
    if (!state) return;
...
export function makeLyricRowLayerGroup(payload, mask, worldW, worldH, pal, motionProfile) {
...
// 网页端没有桌面 Electron 的每帧上传预算治理: remaining 恒为 Infinity。
export var lyricRenderUploadFrameBudget = { frame: 0, remaining: 1, consumed: 0, maxConsumed: 0 };

export var lyricQualityState = {
    queue: [],
    timer: 0,
    idle: 0,
    generation: 1,
...
export function lyricQualityNowMs() {
    return typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
}

export function lyricQualityDisposeTexture(texture) {
    if (!texture) return;
...
export function lyricQualityHasPendingTexture() {
...
export function lyricQualityRecount() {
    var bytes = 0;
...
export function updateLyricQualityStats(tier) {
    lyricQualityRecount();
    // 上游在此汇总 residents 并写入 window.__mineradioLyricQualityStats
    // 调试统计; 本次移植丢弃该导出, 只保留重算副作用。
}

export function releaseLyricRowQuality(row, restoreBase) {
    if (!row) return;
...
export function discardLyricRowPendingQuality(row) {
    if (!row) return;
    var pendingTexture = row.qualityPendingTexture;
    row.qualityPendingTexture = null;
    row.qualityPendingTier = 0;
...
export function invalidateLyricQualityTextures(reason, options) {
    options = options || {};
    ...
    if (lyricQualityState.idle) {
        cancelIdle(lyricQualityState.idle);
...
export function lyricQualityInputPending() {
...
// 显存池预算: 保留「基础池 + 一次原子替换」的形状, 去掉桌面端的硬件档位分支。
export function lyricQualityEffectiveBudgetBytes(tier, oneReplacementBytes) {
...
export function lyricQualityEffectiveMaxRows() {
    return lyricQualityMaxResidentRows() + (Number(lyricQualityState.transitionBudgetUntil) > lyricQualityNowMs() ? 1 : 0);
}

export function lyricQualityEnsureCapacity(extraBytes, pinnedRow, tier) {
...
export function scheduleLyricQualityBuild(delay) {
    ...
    lyricQualityState.idle = requestIdle(run);
}

export function queueLyricRowQuality(data, row, tier, priority) {
...
export function pruneLyricQualityQueue(now) {
...
export function disposeLyricQualityOwner(data) {
    if (!data) return;
...
export function commitLyricRowQuality(row) {
...
export function beginLyricQualitySelectionFrame(deferFinalize) {
...
export function registerLyricQualityCandidates(data, candidates, tier, rootPriority, buildDeferred) {
...
export function registerLyricQualityCommitCandidate(data, row, priority) {
...
export function commitDeferredLyricQualityRows() {
...
export function lyricQualityCandidateTarget(candidate) {
...
export function lyricQualityRowMatchesTarget(row, targetInfo) {
...
export function finalizeLyricQualitySelectionFrame() {
...
// 退化为「始终允许」: 网页端不做每帧上传预算治理, 但保留调用形状与帧计数。
export function resetLyricRenderUploadFrameBudget(deferQualityFinalize) {
    lyricRenderUploadFrameBudget.frame += 1;
    lyricRenderUploadFrameBudget.remaining = Infinity;
    lyricRenderUploadFrameBudget.consumed = 0;
    beginLyricQualitySelectionFrame(deferQualityFinalize);
    // 上游在此写入 window.__mineradioLyricUploadBudgetStats, 本次移植丢弃。
}

export function consumeLyricRenderUploadFrameBudget() {
...
export function updateLyricRowLayers(data, opts) {
