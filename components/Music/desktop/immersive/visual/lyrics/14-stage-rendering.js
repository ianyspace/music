 ...
 * 歌词舞台渲染主模块 —— 整套歌词的调度中心。
 *
 * 移植自上游 Mineradio `public/js/modules/02-visual/14-stage-lyrics-rendering.js`
 * (3207 行)。上游许可证 GNU GPL v3，用户已授权原样复制。
 *
 * 为什么原样搬运：本模块里几乎每一个常数都是整套观感的来源 ——
 * `updateStageLyrics3D` 里 layoutScale / offsetX / offsetY / offsetZ / tiltX /
 * tiltY 的钳制范围、`resetPreparedStageLyricMesh` 的 `mesh.position.set(x, 0.20,
 * 1.46)`、glow 的 follow 系数 (0.26 / 0.24 / 0.22 与 0.92 / 0.90 衰减)、glitch
 * 的抖动系数、resident 行的 z=0.055 基线、星河与各层的缓动…… 它们之间没有
 * 推导关系，改一个数就会破坏整体手感，因此逐行保留，只把 IIFE 的隐式全局
 * 改成显式 import/export。
 *
 * 相对上游的改动仅限:
 *   - 顶层 `function xxx(` → `export function xxx(`，顶层状态对象 `var` 一并 export；
 *   - `fx` `audio` `camera` `uniforms` `lyricsLines` 等运行时状态从 ./runtime.js 取;
 *   - `clampRange` / `normalizeHexColor` 用本仓库 visual/presetData.js 的实现;
 *   - `requestIdleCallback` / `cancelIdleCallback` 走 runtime 的 requestIdle / cancelIdle;
 *   - 桌面客户端的 `window.__mineradioLyric*Stats` 调试导出、`updateStageLyricBuildStats`
 *     埋点、书架 shelfManager 与骷髅嘴合唱分支按移植约定退化（见文件内注释）。
 *
 * 上游许可证 GNU GPL v3。
 */

import { SKULL_PRESET_INDEX, SONIC_PRESET_INDEX, clampRange, normalizeHexColor } from '../presetData.js';
import {
    THREE,
    scene,
    camera,
    ...
    shouldDimWallpaperForShelf,
    shouldOffsetLyricsForShelfDetail,
...
import {
    ...
    lyricCoverWorldQuat,
    lyricTargetQuat,
    applyStageLyricLayoutOffset,
    lyricCameraLockFit,
    setStageLyricViewBasisFromCameraOrQuaternion,
    stageLyricTargetQuaternion,
} from './02-state-layout.js';
import {
    createLyricsParticles,
    updateLyricStarRiver,
    disposeLyricMesh,
    disposeOwnedLyricTexture,
} from './03-star-river.js';
import { normalizeLyricFontKey, lyricFontWeightValue, lyricMeasureTextAtSize } from './05-fonts-texture.js';
import {
    applyLyricPaletteToMesh,
    lyricStageGlowThreeColor,
    getLyricSparkOpacity,
    setLyricSparkOpacity,
    getLyricSparkSize,
    setLyricSparkSize,
    setLyricSparkColor,
} from './07-palette-utils.js';
import {
    normalizeLyricDisplayMode,
    normalizeLyricTranslationMode,
    lyricDisplayLineCountForMode,
    lyricDisplayOffsetsForMode,
    lyricContextOpacityValue,
    lyricContextSpreadValue,
    lyricTranslationGapValue,
    lyricTranslationScaleValue,
    lyricTranslationOpacityValue,
    lyricPrimaryVirtualIndex,
    lyricTranslationVirtualIndex,
    lyricMotionProfile,
} from './08-display-modes.js';
import {
    normalizeStageLyricText,
    normalizeStageLyricPayload,
    cloneStageLyricEntryForLayer,
} from './09-payloads.js';
import { makeLyricMask } from './10-mask-textures.js';
import { makeLyricShaderMaterial, getLyricSunBloomTexture } from './11-lyrics-shaders.js';
import {
    beginLyricRowLayerGroupBuild,
    stepLyricRowLayerGroupBuild,
    finishLyricRowLayerGroupBuild,
    cancelLyricRowLayerGroupBuild,
    updateLyricRowLayers,
    setLyricTextureMaterialOpacity,
    makeLyricRowLayerGroup,
    releaseLyricRowQuality,
    disposeLyricQualityOwner,
    pruneLyricQualityQueue,
    lyricQualityNowMs,
    invalidateLyricQualityTextures,
    resetLyricRenderUploadFrameBudget,
    finalizeLyricQualitySelectionFrame,
    lyricTranslationAnchoredY,
} from './12-row-layers.js';
import {
    buildLyricMesh,
    primeLyricMeshOpacity,
    updateLyricMeshProgress,
    setLyricTrackTarget,
    stableStageLyricRowMaskLayout,
    beginCooperativeLyricMeshBuild,
    stepCooperativeLyricMeshBuild,
    finishCooperativeLyricMeshBuild,
    cancelCooperativeLyricMeshBuild,
} from './13-mesh-build.js';

// ============================================================
//  局部占位: 上游这些名字来自播放器 / 桌面客户端 / 封面粒子 / 相机轨道模块,
//  本项目与 runtime.js 都没有对等物。给同签名的占位, 让下面的调用点保持原样;
//  取不到时全部落到上游的「缺省分支」—— 与这些全局为空时上游的行为一致。
// ============================================================

// 播放队列: 上游 00-state/00-core-stores.js。空队列只影响 trackKey 里的歌名片段。
var playQueue = [];
// 封面粒子组: 上游 00-pointer-cover-particles.js。为空时歌词按世界原点摆放。
var particles = null;
// 相机轨道状态: 上游 01-orbit 模块。骷髅预设的边缘守卫因此恒为 false。
var orbit = { centerLocked: false, recentering: false };
// 骷髅预设的嘴部闪光: 上游 01-float-skull-backcover.js。
var skullBeatFlash = 0;
// 书架 UI: 上游 shelfManager。本项目没有书架, 一律视为未打开。
var shelfManager = null;
// 切歌 token: 上游 05-playback。未定义时 stageLyricRestoreWarmupSeconds 的
// typeof 守卫会自动跳过 token 校验。
// var trackSwitchToken —— 保留上游的 typeof 判断, 不在这里声明。

export var stageLyricPrewarm = { timer: 0, workTimer: 0, workRaf: 0, build: null, mesh: null, key: '', token: 0, targetIndex: null, lightweight: false, dueAt: 0 };
export var stageLyricSingleLinePrewarm = { items: {}, order: [], max: 10 };
export var stageLyricWarmup = { until: 0, reason: '' };
export var stageLyricRestoreWarmup = { time: 0, token: 0, until: 0, reason: '' };
...
export var stageLyricResidentBuild = { job: null, timer: 0, raf: 0, token: 0 };
export var stageLyricResidentDemand = { timer: 0, mesh: null, targetIndex: -1, options: null };
export var stageLyricTrackGeneration = 0;

export function stageLyricColorSignature(value) {
    if (!value) return '';
    if (value.getHexString) return value.getHexString();
    return String(value);
}

export function stageLyricPrewarmStyleKey(options) {
...
export function stageLyricPrewarmBuildGuardKey() {
    return stageLyricPrewarmStyleKey({ omitLivePalette: true });
}

export function stageLyricPreparedKey(payload) {
...
export function stageLyricRenderSignatureForCurrentState() {
...
export function stageLyricCanPreserveSameRender(signature) {
...
export function clearStageLyricFullTrackWarmup() {
    if (stageLyricFullTrackWarmupTimer) {
        clearTimeout(stageLyricFullTrackWarmupTimer);
        stageLyricFullTrackWarmupTimer = 0;
    }
    // 上游用 window.cancelIdleCallback；这里统一走 runtime 的 cancelIdle。
    if (stageLyricFullTrackWarmupIdle && typeof cancelIdle === 'function') {
        cancelIdle(stageLyricFullTrackWarmupIdle);
    }
    stageLyricFullTrackWarmupIdle = 0;
    stageLyricFullTrackWarmupTargetAt = 0;
}

export function cancelStageLyricPrewarmBuildOnly() {
...
export function disposeStageLyricPrewarmMesh() {
...
export function clearStageLyricSingleLinePrewarmItem(key) {
...
export function clearStageLyricSingleLinePrewarmCache() {
...
export function trimStageLyricSingleLinePrewarmCache() {
...
export function stageLyricNowMs() {
    // 上游是 window.performance && performance.now ? performance.now() : Date.now();
    // 这里加 typeof 守卫, 与 03-star-river.js 的写法保持一致, 避免 SSR 期崩溃。
    return typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
}

export function stageLyricTextLoadInfo() {
...
export function stageLyricPreferLightweightTrack() {
    var info = stageLyricTextLoadInfo();
    return !!(info && info.preferLightweight);
}

// 上游恒 false（桌面客户端的整轨预热治理），保留函数形状与调用点。
export function stageLyricShouldSkipFullTrackWarmup(reason) {
    if (!stageLyricPreferLightweightTrack()) return false;
    return false;
}

export function clearStageLyricRestoreWarmup() {
...
export function stageLyricRestoreWarmupSeconds() {
...
export function stageLyricIndexForSeconds(seconds) {
...
export function scheduleStageLyricRestorePrewarm(reason, delay) {
...
export function requestStageLyricRestoreWarmup(seconds, token, reason) {
...
export function requestStageLyricWarmup(reason, ms) {
...
export function clearStageLyricWarmup() {
    stageLyricWarmup.until = 0;
    stageLyricWarmup.reason = '';
}

export function invalidateStageLyricPayloadForNewLyrics(reason) {
...
export function stageLyricWarmupPending() {
...
export function stageLyricPayloadIsSingleLine(payload) {
    if (!payload) return false;
    return normalizeLyricDisplayMode(payload.mode || (fx && fx.lyricDisplayMode)) === 'single';
}

export function lyricVerticalFloatEnabled() {
    return !fx || fx.lyricVerticalFloat !== false;
}

// 12-row-layers.js 按上游原名 import 这个函数, 这里额外给一份 stage* 前缀别名。
export const stageLyricVerticalFloatEnabled = lyricVerticalFloatEnabled;

export function stageLyricSingleLineTrackStub(index) {
    var idx = Math.max(0, Math.round(Number(index) || 0));
    return { entries: [], activeLine: 0, start: idx, end: idx, lightweight: false };
}

export function stageLyricSingleLineNextIndex(index) {
...
export function stageLyricSingleLineNextPrewarmReady(currentIndex) {
    var nextIndex = stageLyricSingleLineNextIndex(currentIndex);
    return stageLyricSingleLineIndexPrewarmReady(nextIndex);
}

export function stageLyricSingleLineIndexPrewarmReady(index) {
...
export function stageLyricSingleLineUpcomingIndexes(index, count) {
    var out = [];
...
export function stageLyricSingleLinePrewarmDelay(currentIndex, targetIndex, lyricTime, slot) {
...
export function stageLyricSingleLinePrewarmCanServePayload(payload) {
...
export function takeStageLyricSingleLinePrewarmMesh(payload) {
...
export function scheduleStageLyricSingleLineCachePrewarm(index, reason, delay) {
...
export function scheduleStageLyricSingleLineNextPrewarm(currentIndex, lyricTime, reason) {
...
export function scheduleStageLyricSingleLineBootstrapPrewarm(reason, delay) {
    if (!fx || !fx.particleLyrics || !lyricsLines || !lyricsLines.length) return false;
...
export function resetPreparedStageLyricMesh(mesh, payload, lineStep) {
    if (!mesh) return null;
    payload = normalizeStageLyricPayload(payload);
...
export function takeStageLyricPrewarmMesh(payload) {
    var key = stageLyricPreparedKey(payload);
...
export function stageLyricMeshCanServePayload(mesh, payload) {
...
export function stageLyricPrewarmCanServePayload(payload) {
    if (stageLyricSingleLinePrewarmCanServePayload(payload)) return true;
    var key = stageLyricPreparedKey(payload);
...
export function stageLyricCurrentMeshAlreadyPreparedForPayload(payload) {
...
export function stageLyricPreparedFullPayloadForIndex(index) {
...
export function stageLyricPrewarmFullCanServeIndex(index) {
    return !!stageLyricPreparedFullPayloadForIndex(index);
}

export function shouldDeferStageLyricSyncBuild(payload, redrawOnly) {
...
export function requestStageLyricDemandPrewarm(payload) {
    if (!payload) return;
...
export function ensureStageLyricPlaybackWarmup(reason, delay) {
    if (!fx || !fx.particleLyrics || !lyricsLines || !lyricsLines.length) return false;
...
export function requestStageLyricLightweightUpgrade(reason, delay) {
    if (!fx || !fx.particleLyrics || !lyricsLines || !lyricsLines.length) return false;
    var now = stageLyricNowMs();
    if (
...
export function stageLyricCurrentUsesPersistentTrack() {
...
export function stageLyricResidentRowKey(row) {
    if (!row) return '';
...
export function updateStageLyricPersistentResidentBounds(data) {
...
export function cancelStageLyricResidentBuild() {
...
export function cancelStageLyricResidentDemand() {
...
export function scheduleStageLyricResidentDemand(mesh, targetIndex, options) {
...
export function buildStageLyricResidentPayload(index, start, end, options) {
    options = options || {};
    if (!lyricsLines || !lyricsLines.length) return null;
    var mode = normalizeLyricDisplayMode(fx && fx.lyricDisplayMode);
...
export function disposeStageLyricResidentRow(row) {
    if (!row) return;
...
export function alignStageLyricResidentEffectToRow(row, effect, zOffset) {
...
export function stageLyricResidentDisplayedScrollOffset(data, fallbackOffset) {
...
export function stageLyricResidentTransformSnapshot(data) {
    data = data || {};
...
export function primeStageLyricResidentRowTransform(data, row, transformSnapshot) {
...
export function mergeStageLyricResidentBundle(mesh, bundle) {
...
export function finishStageLyricResidentBuild(job) {
    ...
    // 上游此处写 window.__mineradioLyricResidentStats 调试统计, 本项目不需要。
}

export function runStageLyricResidentBuild(job) {
    stageLyricResidentBuild.timer = 0;
    stageLyricResidentBuild.raf = 0;
...
export function scheduleStageLyricResidentBuildWork(job, delay) {
    if (!job || stageLyricResidentBuild.job !== job) return;
...
export function startStageLyricResidentBuild(mesh, targetIndex, start, end, options) {
    options = options || {};
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function stageLyricPersistentPrimaryMap(data) {
    var map = {};
...
export function stageLyricPersistentResidentRowMap(data) {
    var map = {};
    if (!data || !Array.isArray(data.rowLayers)) return map;
    for (var i = 0; i < data.rowLayers.length; i++) {
        var row = data.rowLayers[i];
...
export function stageLyricPersistentLineRowsResident(data, lineIndex, rowMap) {
...
export function stageLyricPersistentLineEffectsResident(data, lineIndex, rowMap) {
    lineIndex = Math.round(Number(lineIndex));
    if (!isFinite(lineIndex) || !lyricsLines || lineIndex < 0 || lineIndex >= lyricsLines.length) return true;
    var entry = stageLyricTrackBaseEntry(lineIndex);
    if (!entry) return true;
...
export function stageLyricPersistentTargetRowsReady(mesh, targetIndex, options) {
    options = options || {};
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function stageLyricPersistentTargetEffectsReady(mesh, targetIndex) {
    return stageLyricPersistentTargetRowsReady(mesh, targetIndex, { effects: true });
}

export function stageLyricPersistentNextTextRunwayRange(data, targetIndex, rowMap) {
...
export function commitStageLyricPersistentPendingTarget(mesh) {
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function ensureStageLyricPersistentTrackRows(mesh, targetIndex, options) {
    options = options || {};
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function trimStageLyricPersistentTrackRows(mesh, targetIndex, options) {
    options = options || {};
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function initializeStageLyricPersistentTrack(mesh, payload) {
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function stageLyricCurrentUsesLightweightTrack() {
    var data = stageLyrics && stageLyrics.current && stageLyrics.current.userData && stageLyrics.current.userData.lyric;
    return !!(data && data.trackLightweight);
}

export function stageLyricTrackRevealReady(mesh) {
    var data = mesh && mesh.userData && mesh.userData.lyric;
...
export function stageLyricShouldHoldOutgoingForReveal(current, incoming) {
...
export function releaseStageLyricRevealHoldsForSuccessor(successor) {
...
export function stageLyricLightweightPrewarmAwaitingTakeover() {
...
export function stageLyricCurrentCanResumeWithoutWarmup() {
...
export function upgradeCurrentStageLyricFromPreparedTrack(reason) {
...
export function stageLyricProgressPreviewActive() {
    return typeof isProgressDragPreviewActive === 'function' && isProgressDragPreviewActive();
}
export function stageLyricPlaybackSeconds() {
...
export function stageLyricProgressSeekVisualReady(seconds) {
...
export function chooseStageLyricPrewarmIndex() {
    if (!lyricsLines || !lyricsLines.length) return -1;
    if (
...
export function stageLyricSingleLineBootstrapIndex() {
    if (!lyricsLines || !lyricsLines.length) return -1;
...
export function stageLyricShouldBuildCooperatively(payload) {
    return !!(
...
// 上游写 window.__mineradioLyricBuildStats 的桌面客户端性能埋点。本项目不需要
// 这个调试对象, 保留函数形状与调用点, 函数体留空。
export function updateStageLyricBuildStats(job, active, chunkMs) {
    return;
}

export function finishStageLyricCooperativePrewarm(job) {
    ...
    try {
        ...
        console.error('[lyric-finish-build]', e && (e.stack || e.message)); // TEMP
        cancelStageLyricPrewarmBuildOnly();
        return;
    }
    stageLyricPrewarm.build = null;
...
// 上游依赖 navigator.scheduling.isInputPending (仅 Chromium)。判断保留, 但加
// 存在性判断, 取不到时与上游 catch 分支一致返回 false。
export function stageLyricShouldYieldToPendingInput() {
    try {
        return !!(
...
export function stageLyricCooperativeNextDelay(job, chunkMs) {
...
export function scheduleStageLyricCooperativeWork(job, delay) {
    if (!job || stageLyricPrewarm.build !== job) return;
    delay = Math.max(0, Number(delay) || 0);
...
export function runStageLyricCooperativePrewarm(job) {
    stageLyricPrewarm.workTimer = 0;
    if (!job || stageLyricPrewarm.build !== job) return;
    if (
    ...
    try {
        ...
        console.error('[lyric-step-build]', e && (e.stack || e.message)); // TEMP
        cancelStageLyricPrewarmBuildOnly();
        return;
    }
    var chunkMs = stageLyricNowMs() - startedAt;
...
export function startStageLyricCooperativePrewarm(payload, key, token, lightweight, reason) {
    cancelStageLyricPrewarmBuildOnly();
    ...
    // 上游此处写 window.__mineradioLyricBuildStats 调试统计, 本项目不需要。
    scheduleStageLyricCooperativeWork(job, 0);
}

export function scheduleStageLyricPrewarmForIndex(targetIndex, reason, delay) {
    ...
    stageLyricPrewarm.timer = setTimeout(function () {
        ...
        try {
            ...
            console.error('[lyric-prewarm-build]', e && (e.stack || e.message)); // TEMP
            disposeStageLyricPrewarmMesh();
        }
    }, wait);
}
export function scheduleStageLyricPrewarm(reason, delay) {
    var singleLineIndex = stageLyricSingleLineBootstrapIndex();
    scheduleStageLyricPrewarmForIndex(singleLineIndex >= 0 ? singleLineIndex : null, reason, delay);
}
export function stageLyricMultiLineWarmupLoad() {
    var mode = normalizeLyricDisplayMode(fx && fx.lyricDisplayMode);
    return mode !== 'single';
}
export function stageLyricFullTrackWarmupDelay(delay, reason) {
...
export function queueStageLyricFullTrackWarmupRetry(reason, delay) {
...
export function runStageLyricFullTrackWarmup(reason) {
    ...
    // 上游用 window.requestIdleCallback；这里统一走 runtime 的 requestIdle。
    if (stageLyricMultiLineWarmupLoad() && typeof requestIdle === 'function') {
        stageLyricFullTrackWarmupIdle = requestIdle(function () {
            stageLyricFullTrackWarmupIdle = 0;
            run();
        });
    } else run();
...
export function scheduleStageLyricFullTrackWarmup(reason, delay) {
    if (stageLyricCurrentUsesPersistentTrack()) {
        ensureStageLyricPersistentTrackRows(stageLyrics.current, stageLyrics.currentIdx, { reason: reason || 'persistent-track-warmup' });
        return true;
    }
...
export function showStageLine(text, redrawOnly, options) {
    options = options || {};
    createLyricsParticles();
...
export function refreshCurrentLyricStyle() {
    cancelStageLyricResidentBuild();
    clearStageLyricSingleLinePrewarmCache();
    disposeStageLyricPrewarmMesh();
...
export function clearStageLyrics() {
...
export function stageLyricUsesSingleLineSwap(mesh) {
...
export function updateStageLyrics3D(dt) {
    ...
    // 骷髅嘴合唱 (skullParticleGroup / skullLyricMouth*) 本项目不需要, 恒为 false,
    // 下面对应的分支保留原数值但不会执行。
...
export function lyricKaraokeMeasureContext() {
...
export function lyricKaraokeMetricsKey(line) {
    return [
        line && line.text || '',
...
export function lyricKaraokeWordRanges(line) {
...
export function lyricLineHasNativeKaraoke(line) {
    return !!(line && Array.isArray(line.words) && line.words.length && Number(line.charCount) > 0);
}
export function getLyricLineProgress(line, nextLine, now) {
    if (!line) return 0;
...
export function lyricLineDisplayTextAt(index) {
    var line = lyricsLines && lyricsLines[index];
    return normalizeStageLyricText(line && line.text);
}
export function lyricLineTranslationTextAt(index) {
    var line = lyricsLines && lyricsLines[index];
    return normalizeLyricTranslationText(line && line.translation);
}
export function stageLyricContextEntry(index, currentIndex) {
    var line = lyricsLines && lyricsLines[index];
...
export function makeStageLyricTranslationEntry(entry, isCurrent) {
...
export function applyLyricTranslationModeToEntries(entries, activeLine, maxRowsOverride) {
...
export function applyLyricTranslationModeToTrackEntries(entries, activeLine, maxRowsOverride) {
    entries = Array.isArray(entries) ? entries : [];
    activeLine = Math.max(0, Math.min(entries.length - 1, activeLine || 0));
    var mode = normalizeLyricTranslationMode(fx && fx.lyricTranslationMode);
    if (mode === 'off' || !entries.length) return { entries: entries, activeLine: activeLine };
...
export function stageLyricTrackKeyForMode(mode) {
...
export var stageLyricTrackCache = { key: '', entries: null, lineMap: null, start: 0, end: -1 };
export function stageLyricTrackBaseEntry(index) {
    var line = lyricsLines && lyricsLines[index];
    var text = lyricLineDisplayTextAt(index);
    if (!text) return null;
    return {
...
export function lyricMeshTrackWindow(index, mode, options) {
    options = options || {};
...
export function buildStageLyricMeshTrackEntries(index, mode, options) {
    mode = normalizeLyricDisplayMode(mode);
...
export function lyricBufferedTrackWindow(index, mode) {
    var last = lyricsLines && lyricsLines.length ? lyricsLines.length - 1 : -1;
    if (last < 0) return { start: 0, end: -1 };
...
export function buildStageLyricTrackEntries(index, mode) {
    mode = normalizeLyricDisplayMode(mode);
    if (!lyricsLines || !lyricsLines.length || index < 0) return { entries: [], activeLine: 0, start: 0, end: -1 };
...
export function buildStageLyricDisplayPayload(index, options) {
    options = options || {};
    var mode = normalizeLyricDisplayMode(fx && fx.lyricDisplayMode);
...
export function buildStageLyricPlaybackPayload(index) {
...
export function findStageLyricIndexAtTime(t) {
    if (!lyricsLines || !lyricsLines.length) return -1;
...
export function retireCurrentStageLyricForIdle() {
...
export function resetStageLyricResumeFrameGates() {
    var now = stageLyricNowMs();
...
export function markStageLyricsPlaybackResume(reason) {
...
export function tickLyricsParticles() {
    ...
    if (newIdx < 0) {
        // 上游这里是函数 currentLyricFallbackText(); runtime 导出的可能是字符串,
        // 沿用上游到处使用的 typeof === 'function' 守卫。
        var introText = typeof currentLyricFallbackText === 'function' ? currentLyricFallbackText() : currentLyricFallbackText;
        if (!introText) {
            clearStageLyrics();
            return;
        }
...
export function disposeLyricsParticles() {
    clearStageLyrics();
