/**
 * 移植自上游 Mineradio public/js/modules/02-visual/11-lyrics-shaders.js。
 * 上游许可证 GNU GPL v3（用户已授权原样复制）。
 * 这里逐行保留着色器逻辑、颜色与全部观感数值，仅做 ESM 导入导出适配，
 * 避免重写或优化改变原有效果。
 */

import { THREE, uniforms, lyricsHasNativeKaraoke } from './runtime.js';
import {
    lyricBeatGlowThreeColor,
    lyricStageGlowThreeColor,
    lyricThreeColor,
} from './07-palette-utils.js';
import { lyricMotionProfile } from './08-display-modes.js';

var lyricSunBloomTexture = null;
export function getLyricSunBloomTexture() {
...
export function makeLyricShaderMaterial(mask, pal, motionProfile) {
