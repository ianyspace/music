 ...
 * 歌词调色板 / 文本工具。
 *
 * 原样移植自上游 Mineradio `public/js/modules/02-visual/07-lyrics-palette-text-utils.js`
 * (576 行)。上游许可证 GNU GPL v3, 已获授权复制。
 *
 * 为什么逐行搬运而不重写: 本文件里几乎所有数值都是调过的观感参数 ——
 * 高亮色的最小饱和度 minS、亮度抬升量 +0.30、封面取色的 chroma 阈值
 * (0.055 / 0.026 / 0.095 / 0.014)、区域分桶步长 24、评分权重 1.6 / 0.45
 * 等等。它们之间没有推导关系, 改一个数就会破坏整体配色观感, 因此
 * 保留原变量名与常数, 只把 IIFE 的隐式全局改成显式 import。
 *
 * 上游许可证 GNU GPL v3。
 */
...
import { rgbToHsl, hslToRgb, rgbCss } from './helpers.js';
import { clampRange, normalizeHexColor } from '../presetData.js';
import { stageLyrics, lyricSunColor, lyricSunHotColor } from './02-state-layout.js';
import { lyricMeasureText, hexToRgb } from './05-fonts-texture.js';
import { normalizeLyricDisplayMode, normalizeLyricTranslationMode } from './08-display-modes.js';
import { stageLyricPrewarm } from './14-stage-rendering.js';

// ---------------------------------------------------------------- 局部占位
// 上游 `playQueue` / `playlist` 来自 00-state/00-core-stores.js, `songCoverSrc`
// 来自 05-playback/01-cover-custom-map.js —— 两者都是播放器模块, 本仓库还没有
// 对等实现, 也没有对应的兄弟模块可 import。这里给同签名的局部占位, 只影响
// 封面调色板的缓存 key: 拿不到歌曲时返回空串, 调色板照常按当前封面重算。
const playQueue = [];
const playlist = [];
const songCoverSrc = function (song) {
    if (!song) return '';
    return String(song.cover || song.customCover || '');
};

export function uiAccentHex(fallback) {
...
export function lyricPaletteFromHex(hex) {
    var c = hexToRgb(hex);
...
export function silverBlueLyricPalette() {
    return {
        primary: '#d8f1ff',
        secondary: '#9db8cf',
        highlight: '#eef7ff',
...
export function setLyricSparkOpacity(data, value) {
...
export function getLyricSparkOpacity(data) {
...
export function setLyricSparkSize(data, value) {
    if (!data || !data.sparkMat) return;
...
export function getLyricSparkSize(data) {
...
export function lyricMultiLineGlowDetached() {
...
export function lyricDetachedGlowThreeColor(fallback, minLum) {
...
export function setLyricSparkColor(data, color) {
    if (!data || !data.sparkMat) return;
...
export function effectiveLyricPalette(pal) {
...
// 上游是 IIFE 顶层 var(隐式全局)。这里只在本文件内使用, 不导出。
var lyricPaletteTween = null;
export function lerpStageLyricPaletteColor(fromCss, toCss, t, fallback, minLum) {
...
export function mixStageLyricPalette(from, to, t) {
    from = from || {};
    to = to || {};
    return {
...
export function applyStageLyricPaletteNow(pal) {
...
export function setStageLyricPalette(pal, opts) {
...
export function lyricTextPaletteFromHsl(hsl, avgL, chroma, opts) {
    opts = opts || {};
...
export function lyricCoverSampleCss(sample, fallback) {
...
export function lyricCoverSample(r, g, b, score, lum, chroma, hsl) {
    return {
        score: score,
        r: r,
        g: g,
...
export function lyricCoverLooksMonochrome(stats) {
    stats = stats || {};
...
export function lyricCoverPushUniqueColor(list, value) {
...
export function lyricCoverAreaDistance(a, b) {
    if (!a || !b) return 1;
...
export function lyricCoverAddAreaBucket(buckets, r, g, b) {
    var step = 24;
    var key = [
        Math.round(r / step),
        Math.round(g / step),
...
export function lyricCoverAreaBucketList(buckets) {
...
export function lyricCoverPickAreaColor(list, test, fallback, avoid, minDistance) {
...
export function lyricCoverAreaPaletteFromBuckets(buckets) {
...
export function lyricCurrentCoverPaletteKey() {
    try {
        var song = null;
...
export function updateLyricPaletteFromCover(coverCanvas) {
    ...
    try {
        ...
        // 桌面客户端的 sonic workshop 面板, 网页端没有 window 上的实现时跳过。
        if (typeof window !== 'undefined' && window.MineradioSonicWorkshop && typeof window.MineradioSonicWorkshop.pushProperties === 'function') window.MineradioSonicWorkshop.pushProperties(true);
    } catch (e) { }
}

export function wrapLyricText(ctx, text, maxWidth, maxLines, fontSize) {
...
export function cssColorToThreeColor(css, fallback) {
...
export function lyricThreeColor(css, fallback, minLum) {
