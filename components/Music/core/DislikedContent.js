...
import styles from './DislikedContent.module.scss';

/**
 * 不喜欢歌曲 — the body of the keep-out list's manager.
 *
 * The *content* half of the panel: both layouts render it, each inside its own
 * chrome, which is why it returns a fragment (the panel's chrome is a flex
 * column and these blocks have to be its direct children) and why nothing here
 * knows how the panel arrives on screen.
 *
 ...
 */
const DislikedContent = function ({
    keys,
    tracks,
    onRestore,
}) {
...
export default DislikedContent;
