...
| `components/Music/h5/` | 手机端：`MusicApp` 外壳 + 列表/播放页/迷你条/我的/账号 + 三个底部面板 |
...
| `components/Music/`（根） | 只放共享件：`Cover` / `Marquee` / `icons` / `shared` / `audioCache` / `librarySource` / `playStats` |
| `cloudflare-worker/schema.sql` | D1 播放次数表的建表语句（Worker 自己也会跑一遍同样的 DDL） |
...
- **列表偏好有两个，形状一样但不是一类**（`shared.js` 的 `ORDER_KEY` / `LIKED_KEY`）：
  `order` 是置顶排名、`liked` 是保留表。都是 `<source>:<id>` 的 JSON 数组，
  都走 `readKeyList` / `writeKeyList`，都在 `localStorage`。
  两个一起进 `visibleTracks`，顺序是：`applyPinnedOrder`（只排序）→ 只看喜欢 → 搜索。
  ...
  **两个都只排序或收窄，没有一个会删行** —— 这正是空列表文案能少一个分支的原因。
  ...
  「永不过期」是字面意思：这里没有 `expiresAt` 可读 —— 列表缓存有，所以它得处理旧值，这个不用。
...
- **手机端顶部栏里是头像，不是曲库名**（`h5/TrackList.js` 的 `<header>`）。品牌图标和标题
  被拿掉了，换成访客头像 —— 它同时是**进入「账号」页的按钮**（见下一条）：
  ...
  - **盘子上不再有毛玻璃遮罩，而且是有意去掉的**：曾经有一层 `.avatar-glass`
    ...
    `--avatar-bg` 是这件事的全部。随遮罩一起走掉的还有 `.avatar` 上的 `position` /
    `isolation` —— 它们分别是为了给绝对定位的子元素当锚、把它的混合关在盘子里。
    （当初定 2px 那个半径的办法：0/2/3/5px 排一列、浅深各一遍出图比。
    `backdrop-filter` 的半径只有像素能回答，断言看不见 —— 以后若再要这层，量具得重写。）
  ...
  - **头像现在是按钮，因为背后终于有东西了**：点它进「账号」页（`h5/Account.js`，
    第三个 tab）。它曾经**故意不是**按钮 —— 那时它背后什么都没有，而一个点了没反应的控件
...
- **「账号」页（`h5/Account.js`）是访客自己的页面**，从列表左上角头像进入，和「我的」
  （`h5/Profile.js`）分工明确：**「我的」管「这里有哪些歌」，「账号」管「听歌的是谁」**。
  它装了原来列表 ⋮ 抽屉里的 `缓存管理` 和 `切换外观`，加上 QQ 号绑定和 `听歌排行`（见下）。抽屉里**只留**
  `谷歌云盘链接`：⋮ 长在曲库上，而个人设置和曲库不是一类东西 —— 一个抽屉里混着两类动作，
  访客就得先想「我要找的那个大概在哪一类里」。
  - **QQ 号做两件事：换头像，和当听歌排行的归属**（`shared.js` 的 `QQ_KEY` / `normalizeQq` /
    `qqAvatarUrl`）。合法就是 5–11 位数字，**只看长度**：没有校验位、没有前缀规则，也没法从
    这里验证账号是否存在 —— 再严一点就是编造这个应用没有的权威，它是访客给自己头像起的
    名字，不是凭据。这条在加了排行之后**更重要了**：`GET /stats?qq=…` 因此没有鉴权，
    知道号码的人就能看它的排行（见「听歌排行」一节）。输入先剥掉非数字（有人会粘
    「QQ：1234567」），因为拒绝这种输入教不了任何人任何事。
  - **号码本身归 `usePlayer`，不是归这个页面**（`usePlayer` 的 `qq` / `saveQq`）。它以前是
    `h5/MusicApp` 的 state，那时它只服务于头像；现在它同时是播放上报的 key，而上报发生在
    `usePlayer` 里（三套布局共用），一个值两处读就是「谁在听」有两个答案。读取放在 effect
    里而不是 `useState` 初值里（静态导出，SSR 会先跑一遍），`''` 是一个真值（清除写的），
    所有读者都把它读成「没绑定」而不是「还没读到」。
  ...
  - **URL 和「加载失败」这个标志都归外壳**（`MusicApp`），两个页面（列表头像、账号页大头像）
    只拿同一个结果：`''` 就是「画音符」。**一个兜底由两个调用方各自决定，就是两个兜底**，
    迟早会不一致 —— 其中一处显示 QQ 头像、另一处显示音符，是最难查的那种 bug。
    （号码本身现在从 `usePlayer` 来，见上一条；这里说的是图片 URL 和失败标志。）
  - **「清除」必须有**：号码是访客自己敲进去的，就得能撤回。和 `取消置顶` 同一类 ——
    单向操作（只能设、不能撤）是这个项目已经踩过一次的坑。
  - **`Account.module.scss` 是 `Profile.module.scss` 的复制**（page / head / title / nav-btn /
    group / row / footnote 那一套形状），**有意不抽公共模块**：两个页面还不值得第三个两边
    都 import 的文件，而且那个文件会落在一个谁也不属于的地方。**出现第三个设置风格页面时
    就是抽取的时机**，那时把「我的」一起折进去，而不是留一个页面用旧副本。
  - **它的 tab 名是 `account`**（`MusicApp` 的 `tab`），迷你条仍然只在 `list` 上出现 ——
    账号页是设置，不是播放界面。

### 听歌排行（播放次数）

**这是这个应用里唯一会离开浏览器的数据。** 别的都在本机（音频缓存、喜欢、置顶、QQ 号），
播放次数不行 —— 它天生是「跨设备看同一个人」的功能，所以要有个不在浏览器里的地方：
Cloudflare D1，由 `cloudflare-worker/` 的 `POST /plays` / `GET /stats` 读写。

三个文件，各管一段：

| 文件 | 管什么 |
| --- | --- |
| `components/Music/playStats.js` | 本机待同步日志、非阻塞上报、手动同步、拉统计、`formatAgo` |
| `components/Music/h5/usePlayStats.js` | 「账号」页那张卡片的状态（加载 / 错误 / 待同步 / 同步） |
| `cloudflare-worker/src/index.js` + `schema.sql` | 落库、去重、两个窗口的排行查询 |

**三条不变量，改任何一处之前先想清楚会不会破掉它们：**
...
**排行卡片的两份数据来自两个查询**（全部 / 最近 7 天是同一个 SQL 换 `WHERE`），
所以卡片上那行汇总必须跟着当前 tab 走，不能用同一个 total 写两遍。
列表为空时**不画汇总**（「共播放 0 次」和「还没有播放记录」是同一句话说两遍）。
「同步」按钮是**一个按钮两种长相**（有本地记录时是实心、没有时是描边），和
`置顶` / `喜欢` 同形：日志空着的时候还杵一个大红按钮，是在邀请一次什么也不做的点击。

**没有鉴权，是有意的。** QQ 号不是凭据（见「账号」页那几条），所以 `GET /stats` 回答的是
「这个号码听过什么」。要保护它得先有登录，不是在接口上加个参数。这条写在 README 里给
访客看，也写在 `cloudflare-worker/README.md` 里给改的人看。
