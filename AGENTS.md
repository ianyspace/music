...
- **`.bar-row` 的上下 padding 相等**（`16px 30px`），40px 的内容行正好落在 72px 胶囊的
  中线上 —— 和手机端迷你条「58px 里放 50px 唱片、上下各 3px」是同一种算法。
  它曾经是 `24px 30px 8px`：上边 24px 是给进度条让出来的带子、下边只有 8px 余量，
  于是整行都偏下（最先被看出来的是那枚实心红播放键）。**这个 padding 就是
  「播放键居中」的全部实现**，改回不对称就又会歪。
- **`.seek` 绝对定位，盒子 12px 高、`top: 4px`，正好在内容行上沿（16px）结束**：
  进度条不再从行内边距里借带子，所以 padding 才能做成对称的。12px 的滑块
  （`margin-top: -4.5px` 对着 3px 轨道）填满这个盒子，拖到哪儿都盖不住控件，
  离唱片（34px，从 19px 开始）也还有 3px。左右各让开 30px 是因为胶囊圆角 999px，
  行内边距小于半径时内容会跑到填充外面去。
