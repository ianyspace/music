...
- **`.ctrl-play` 的 `padding` 是 0**：它抄过手机端播放键的 `padding-left` 光学微调，
  但手机端那颗是**透明**按钮，桌面端是一枚实心红圆 —— 同样的 1px 在圆里是量得出来的。
...
- **列表那一列顶部是一行 36px 方块**（`.side-head`）：应用图标 `.side-mark`、
  折叠开关 `.side-toggle`、只看喜欢 `.side-heart`、搜索（关着的 `.search-btn` /
  展开后的 `.search`）。四个都是 36px、圆角都是 12px，所以开合搜索时列表一个像素都不动。
  - **搜索必须排在最后一个**：它是唯一会变成 `flex: 1` 的那一个（点开就是输入框），
    排在行尾才会往右长进空着的那半边 —— 前面的图标、开关和红心一个像素都不动。
    手机端把输入框放在图标和右侧操作**之间**，那是 390px 宽、三个控件时的做法；
    这里列宽 312px，能长大的只能是它。
...
- **红心借的是运输控件自己的类**（`.ctrl-btn` + `.ctrl-on`），没有自己的规则：
  ...
  **故意不涂 `--accent`**，而手机端播放页里那颗被喜欢的心是涂红的：播放条有一条写死的
  规矩 —— 整条里唯一的红是播放键和进度条的填充（见 `.ctrl-on` 的注释），
  一颗实心的红心加上那枚红圆，就会把这条胶囊变回「一颗药里两套配色」。
