...
- **手机端顶部栏会滚走，浮动条是它的替身**（`h5/TrackList.js` 的 `<header>` + `h5/MusicApp.js`
  的 `.float`）。header **不是** sticky：它跟内容一起滚走，滚走之后 shell 才浮出毛玻璃的一条 ——
  左边头像、右边三个按钮装在同一个胶囊里。几条容易漏的：
  - **观察的是 header 本身，不是滚动距离**：`IntersectionObserver` + `threshold: 0`，
    最后一个像素离开就是「滚走了」。header 多高是它自己的事，写死一个 offset 会在列表短到
    那个位置还停在 header 中间时开始断言错的东西。
  - **`headGone` 记的是 `{ tab, gone }`，不是一个布尔**：profile tab 下 header 是 `display: none`，
    没有盒子的元素会被观察者读成「不在视口」，裸布尔会在切回列表时闪一下浮动条。
  - **浮动条一直挂在文档里、用 class 显隐**，所以「在不在」是错的问题 —— 隐藏时它也在。
    实现和断言看的都是 `visibility`：淡出时延迟到淡出结束（`visibility 0s linear 0.26s`），
    淡入时立刻 —— 这样退出丝滑，而隐藏期间它既不进 tab 序也不进无障碍树。
  - **它必须渲染在 shell 根部**，理由和抽屉一样：列表列在带 `transform` 的 tab 过渡容器下面，
    `position: fixed` 的后代会按那个祖先定尺寸。`left: 0; right: 0; margin: 0 auto; width: 480px`
    是为了和 480px 的应用列对齐，好让动画只剩一个 `translateY`。
  - **三个按钮只定义一次**（`h5/ListActions.js`）：header 和浮动条同时只有一处可见，
    但它们是同一批控件读同一份状态，抄一份就是给文案、`aria-pressed`、显隐条件各留一个漂移的地方。
    只有 `showSearch` 两边不同 —— 搜索框展开时 header 的那个按钮让位（输入框**就是**那个槽位），
    浮动条的那个留着（它那儿没有输入框，点它就是回到上面那个）。
  ...
  - **搜索框的开合状态在 shell 里**，不在 `TrackList`：浮动条也要能展开它，而输入框在 header 里。
    从浮动条点搜索会**同时滚回顶部**，否则看起来就是一个没反应的按钮。
...
- 浮动条/抽屉挂在列表列里面而不是 shell 根部 → `position: fixed` 被祖先的 `transform` 收进那一列，
  跟着列表一起滚走，样式还在、位置全错。
...
（反过来的例子也在：手机端顶部的浮动条**只有手机端有**，那是设计，不是漏。）
...
它断言八件事，按页面在 `PAGES` 表里配：不自动播放、列表真的到了、点一行真的在播、
快捷键真的做了它说的事、两个开关真的翻转了、行抽屉里的动作真的生效、手机端顶部栏真的交班、
没有任何异常。列表和播放都是**轮询**等而不是睡固定秒数，并把等到用了多久打出来 ——
于是「慢」和「坏」是两个答案。
...
- `top`（9 项）：滚走前浮动条必须是 `visibility: hidden`（它一直挂在文档里，「在不在」不是问题），
  滚走后 header 离开视口、浮动条可见；胶囊里的按钮和 header 里的**整体相等**，
  点胶囊里的搜索真的展开 header 的输入框并回到顶部。
- `like`（21 项）：从行抽屉喜欢 → 只看喜欢只剩那一首 → **`Page.reload` 之后还在** →
  播放页那颗心点亮并能取消 → 空的我喜欢有自己的文案 → Drive 曲库下整套入口消失。
  **这是唯一会重载页面的一段，所以它放在最后**，否则它会把前面所有检查测量的页面换掉。
