...
- **手机端顶部栏里是头像，不是曲库名**（`h5/TrackList.js` 的 `<header>`）。品牌图标和标题
  ...
  - **默认头像要的是「Apple 风格」，而那个风格的全部是填色**：一个**填满的盘子**
    （`--avatar-bg`，Apple 的系统灰）+ 白色**挖空**音符（`--avatar-fg`）。浅底 + 深色字形
    在**任何**不透明度下都读成「一个白圈里有个图标」，读不成头像 —— 所以这不是配色偏好，
    是底色和字形各司其职：盘子负责「这是个人」，字形负责「现在还没有人」。
  - **底色是渐变，而且浅端不能太白**：`linear-gradient(150deg, …)`，浅色 `#9a9ba0 → #6a6a6f`、
    深色 `#7c7d82 → #45454a`。字形在正中间，所以它压在斜坡的**中段**上；浅端再白一点
    就会把音符的顶部吃掉。纯色盘子在 40px 上像一块色卡，渐变让它读成一个有光的球面。
  - **盖在盘子上的那层不能是 `--glass`**：60% 白会把盘子重新刷成浅色，也就是「纯白背景
    的默认头像」。14% 白薄光 + `saturate(1.4)` 已经足够读到「隔着玻璃」，盘子还是深的。
