...
- **手机端顶部栏里是头像，不是曲库名**（`h5/TrackList.js` 的 `<header>`）。品牌图标和标题
  被拿掉了，换成访客头像的**占位** —— 一个音乐音符，上面盖一层**低模糊**毛玻璃：
  - **那层玻璃是重点，而且「低」是个窄窗口**：`backdrop-filter` 模糊的是它背后已经画好的东西，
    所以模糊半径**就是**音符能剩下多少 —— 过了 ~4px 它就不再是一个音符，而是一团糊。
    再叠一层 `--glass` 填充才是「玻璃」；只模糊不填色，看起来只是图标失焦了。
