...
- **「账号」页（`h5/Account.js`）是访客自己的页面**，从列表左上角头像进入，和「我的」
  ...
  它装了原来列表 ⋮ 抽屉里的 `缓存管理` 和 `切换外观`，加上 QQ 号绑定。抽屉里**只留**
  `谷歌云盘链接`：⋮ 长在曲库上，而个人设置和曲库不是一类东西 —— 一个抽屉里混着两类动作，
  访客就得先想「我要找的那个大概在哪一类里」。
  - **QQ 号只做一件事：换头像**（`shared.js` 的 `QQ_KEY` / `normalizeQq` / `qqAvatarUrl`）。
    合法就是 5–11 位数字，**只看长度**：没有校验位、没有前缀规则，也没法从这里验证账号是否
    存在 —— 再严一点就是编造这个应用没有的权威，它是访客给自己头像起的名字，不是凭据。
    输入先剥掉非数字（有人会粘「QQ：1234567」），因为拒绝这种输入教不了任何人任何事。
