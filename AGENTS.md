...
- **歌词的三种新样式，挂在「换行」还是挂在「自己的时钟」上，是两种东西**（`h5/NowPlaying.js`
  的 `LYRIC_OPTIONS`，四个样式：普通 / 逐行上浮 / 沉浸单行 / 卡拉OK 扫光）：
  - 页面**唯一的时钟是 `<audio>` 的 `timeupdate`，约 4 Hz**。靠换行触发的样式（上浮、沉浸单行）
    白嫖这个时钟就够；**在两行之间要持续移动的东西**（扫光）用 4 Hz 驱动会看起来像卡住的进度条，
    所以只有它自己开了一个 rAF，直接读那个元素的 `currentTime`。
  - 那个 rAF **写 DOM，不写 state**：每秒 60 次 `setState` 会把整页（唱片、进度条、按钮）
    重渲 60 次，只为挪一个渐变断点。这也是它单独成一个 effect 的唯一理由。
  - 三个新样式**都靠 `lyrics.timed` 兜底**：没有时间戳就没有时钟可跟，退回 `plain`。
    不这么做的话沉浸单行会「只剩最后一行」—— 无时间戳时每行的 `time` 都是 0，
    那个 reduce 会落在最后一行上。
  - `--wipe` 写在**内联**样式上，所以读数要从 `getComputedStyle` 取：内联值只是组件最后一次写的，
    计算值才是真正画出来的（包括第一帧之前样式表里声明的那个 `0%`）。
- **`background-clip: text` 必须关在 `@supports` 里**：不支持的浏览器会把渐变当成实心色条
  **盖在字上**，那一行直接读不出来 —— 不是「没效果」，是「坏掉」。`color: transparent`
  也一起关进去，否则同样会得到一行透明字。
- **渐变要对齐文字，不是对齐那一列**：扫光模式下 `.lyric-active` 是 `width: fit-content` +
  左右 `margin: auto`。块级元素占满整行时，短句的边界会飘在字外面的空白里，边缘就不在字形上。
- **覆盖变体规则时，选择器要原样重复**：上浮的开关是 `.lyrics-rise .lyric-active`（两个类），
...
- **单选组给了 roving `tabIndex`，就必须自己处理方向键**：四个样式行是
  `role="radiogroup"` + `role="radio"`，只有被选中的那一行 `tabIndex=0`。这是「一组一个 Tab 停靠点」
  的正确做法，但**同时**把另外三个从 Tab 顺序里拿掉了 —— 不写 `handleStyleKeys` 的话，
  键盘用户永远够不到它们，而 `role="radio"` 本身就是在承诺方向键可用。
- **扫光的每帧成本量过了：它不慢，慢的是整个歌词页**（`probe-wipe-cost.mjs`，同一首歌、
  同五秒窗口、1× 与 4× CPU 节流、每格跑三次）：
  - 1× 下三种条件（关歌词 / 普通 / 扫光）**分不开**：54.4–55.0 fps、平均帧 17.84–18.08 ms。
    唯一稳定分开的一列是 `RecalcStyleDuration`：0.16–0.19 / 0.16–0.17 / **0.24–0.25**（秒，五秒窗口），
    即每秒六十次写 `--wipe` 多花的是**样式重算**（约 0.3 ms 一帧，占一帧的 1.5%），不是脚本时间 ——
    `ScriptDuration` 三档都在 0.13–0.20，rAF 回调本身几乎免费。`RecalcStyleCount` 三档都是 276：
    **次数没变，单次变贵**，因为被 invalidate 的只有那一行。
  - 4× 下 `普通` 对 `扫光`：fps 34.0–35.8 vs 32.2–36.2、平均帧 27.53–28.93 vs 27.17–30.58 ——
    **区间重叠**，最顺的一次样本还是扫光（36.2）。稳定分开的仍是 `RecalcStyleDuration`
    （0.379–0.444 vs 0.497–0.550）和 `ScriptDuration`（0.493–0.538 vs 0.599–0.693），各多约 0.1 秒 / 五秒。
  - **`longtask` 那一列不能用来比较**：同一个格子跑三次得到 220 / 366 / 56 毫秒，
    而 `record` 自己就是 106 / 326 / 510 —— 它数的是「有没有一个 ≥50 ms 的长任务恰好落进这个窗口」，
    单个 271 ms 的离群样本就能把总数抬到 700 以上。第一次量出来的「扫光 728 vs 普通 222」
    就是这么来的，不是这个样式的属性。
  - **`record` 不是「歌词的底」**，它是另一个场景（一张一直在转的唱片）。1× 下它和歌词页打平，
    4× 下它反而**最慢**（31.0–31.8 fps，歌词两档都在 34 以上）。要问「扫光比普通贵多少」，
    只能拿这两个比，不能拿 `record` 当基线。
...
- **「在动」要至少三帧才证明得了**：卡拉OK 扫光的 `--wipe` 采两次，可能正好跨过一次换行
  （换行会把它清零），读到「没动」并误判成卡住。采三次、任意相邻两次前进即通过。
  另外这个值必须从 `getComputedStyle` 读，理由见上面那条。
- **播放页的抽屉遮罩是 `position: fixed; inset: 0` 的 50% 黑**，所以开着抽屉截出来的图
  是隔着烟玻璃看歌词。要截歌词就先按 Escape 关掉抽屉 —— `.workbuddy-ai/probe-lyric-styles.mjs`
  对四个样式各做一次「选中 → Escape → 截图 → 再打开」。
- **四行文本的样式差异，截图只能证明其中一部分**：上浮是一个 0.3s 的动画，
  等它跑完截图什么也看不到 —— 要轮询当前行的文字，**在换行的那一瞬间**截。
  扫光要两帧。沉浸单行和普通，一帧就够。
- **截歌词行之前要先把它滚到中间，否则截到的是蒙版，不是样式**：`.lyrics` 有一层
  `mask-image`，把上下各 14% 淡到全透明。歌刚开始时「当前行」就是 LRC 的第一行，
  正好坐在这个淡出带上 —— 在那截出来的图里，扫光的亮/暗两色都被压到接近不可见，
  看起来像「渐变根本没生效」。**四张图的 md5 各不相同，肉眼却完全一样**，
  这就是它难发现的地方：文件变了 ≠ 看得出来。做法是先把播放位置跳到曲中，
  再 `scrollTop = offsetTop - (clientHeight - offsetHeight) / 2`，等一帧再截
  （见 `.workbuddy-ai/probe-wipe.mjs`）。
- **要证明「一个渐变真的在画」，钉住变量比等实时值可靠**：暂停播放（组件就不再写
  `--wipe` 了），把 `--wipe` 分别钉在 0% / 35% / 65% / 100%，各截一张 4 倍图。
  0% 必须全暗、100% 必须全亮、中间两张的边界必须在两个不同位置 —— 四张一起才构成证明。
  只截一张实时的，永远分不清「边界刚好在一个字的位置」和「边界根本没在动」。
...
- **`prefers-reduced-motion` 可以用 `Emulation.setEmulatedMedia` 在冒烟测试里模拟**，
  用完 `{ features: [] }` 复位。值得单独断言：覆盖 `.lyrics-rise .lyric-active` 必须写
