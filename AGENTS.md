...
- **手机端顶部栏里是头像，不是曲库名**（`h5/TrackList.js` 的 `<header>`）。品牌图标和标题
  被拿掉了，换成访客头像的**占位** —— 一个音符，上面盖一层**低模糊**毛玻璃：
  ...
  - **底色是彩色渐变，而且两端都不能靠近白**：`linear-gradient(150deg, …)`，
    浅色 `#6a8dff → #8f6bf6 → #c06bef`（蓝 → 紫 → 品红），深色 `#5578f0 → #7a56e6 → #a655dd`。
    白色的音符要压着**整条**斜坡，所以两端都得是饱和的：只要有一端变浅，音符在那一头就没了。
    纯色盘子在 40px 上像一块色卡，渐变让它读成一张图。
  - **盖在盘子上的那层（`--avatar-glass`）和底色是两件事**：底色换过三次
    （灰 → 灰渐变 → 彩色渐变），这层一次都没动，**以后换渐变色也只改 `--avatar-bg`**。
    它不能是 `--glass`：60% 白会把盘子重新刷成浅色，也就是「纯白背景的默认头像」；
    14% 白薄光 + `saturate(1.4)` 已经足够读到「隔着玻璃」。
  - **`IconNote` 的 `filled` 是实心写法，不是描边加填充**：竖线/横梁是填充块、符头是斜椭圆，
    和 SF Symbols 的 `.fill` 一个意思。描边版 21px 时笔画只有 ~1.75px 宽，模糊一到 2px
    就并成一团。列表行里的缩略图仍然用描边版。
  ...
  - **「低模糊」是个窄窗口，而 2px 是比出来的**：`backdrop-filter` 模糊的是它背后已经画好的
    东西，所以模糊半径**就是**音符能剩下多少 —— 过了 ~4px 它就不再是一个音符，而是一团糊。
    这个窗口只有像素能回答，工具是 `node scripts/preview-avatar.js`。
  - **头像不是按钮**：它背后还没有东西，而一个点了没反应的控件比一张不会动的图更糟。
...
`preview-avatar.js` —— 改到相关 UI 时顺手跑一下，尤其是 `emptyListMessage` 那种
「只改了文案」的改动。

`preview-avatar.js` 是给顶部栏那个头像写的，值得说一句为什么它要单独存在：
**「低模糊」是一个只有像素能回答的问题** —— `backdrop-filter` 的半径就是音符能剩下多少，
过 3~4px 它就不再是音符。所以那个脚本把同一个头像按 0/2/3/5px 排成一列、浅色深色各一遍，
改一个数字刷新一次就能比较。截图用 `--force-device-scale-factor=4` 放大**结果**，
**不放大头像本身** —— 半径是 CSS 像素、字形 21px，把头像 `width/height` 调大等于改掉
那个正是问题的比值。断言看不见这种东西。
