...
| `components/Music/h5/` | 手机端：`MusicApp` 外壳 + 两个页面（列表 / 听歌排行）+ 迷你条 + 播放页 + 四个 `SheetChrome` 面板（账号 / 音乐库 / 缓存 / 云盘）+ 两个底部抽屉（⋮ / 行抽屉） |
...
- **列表的 ⋮ 抽屉回来了，而且这次有三个条目**（`h5/MusicApp.js`）：`音乐库`（打开同名的抽屉页，
  也就是原来的「我的」）、`谷歌云盘链接`、`缓存管理`。它中间被删过一次 —— 那时缓存管理和切换外观
  搬去了账号页，抽屉只剩 `谷歌云盘链接` 一个条目，而**只有一个条目的菜单比没有菜单更糟**：
  多一次点击、多一次判断，换来的东西本来可以是一行。现在它装的是**关于这台设备和这个曲库**
  的三件事，和「账号」抽屉页（关于这个人）分工一致 —— **一个菜单值不值得存在，看它是不是
  不止一件事**。
  - 抽屉、`menuOpen` / `menuClosing` / `closeMenu` / `openMenu` / Escape 监听都挂在
    `MusicApp` 这一层（列表列在 `transform` 祖先下，`position: fixed` 会被收进去）。
  - **⋮ 本身不跟 `connected` 走**：曲库空的时候正是最需要 `谷歌云盘链接` 的时候，
    而缓存管理根本不关心列表里有没有歌。
  - **行抽屉（每首歌的 ⋮）不是一回事**：它是逐首的、挂在同一层，理由相同。
  - 三个条目和两个抽屉页共用同一个「面板开合」形状（`MusicApp` 里的 `useSheet`）：
...
- **「账号」抽屉页（`h5/Account.js`）是访客自己的面板**，从列表**最左边**的应用图标升起，
  ...
  「听歌的是谁」**。它装了 QQ 号确认、「我的数据」（`听歌排行` 入口 + `数据同步`）和 `切换外观`；
  `缓存管理` 回到了列表的 ⋮ 抽屉 —— 那是关于设备和曲库的菜单，而这个面板是关于人的。
  ...
  - **文案统一用「确认」，不用「绑定」**，因为那才是访客做的动作（按钮也从 `确定` 改成 `确认`）。
    ...
    名字那一行再重复一遍只会把号码挤到省略号。副标题也不再只讲「缺什么」：
    没号码时它说的是 `没确认 QQ 号：喜欢只存在本机，听歌次数不上传` —— 喜欢**能用**，
    只是留在这台设备上，文案不该读成一扇锁着的门。
  ...
  - **设置页的词汇表已经抽成 `settingsPage.module.scss`**（page / head / title / nav-btn /
    refresh-btn / primary-btn / group / row / hint / footnote）。这三个形状曾经**故意**
    在 Profile / Account 里各抄一份，两边的注释都写着「出现第三个设置风格页面时就是抽取的
    时机」——**「听歌排行」就是那一刻**，于是把「我的」一起折了进来，而不是留一个页面用旧副本。
    后来「账号」和「音乐库」变成了抽屉页：**它们现在只 `composes:` 卡片/行/提示/脚注这套词汇，
    外框改由 `SheetChrome` 给**，`.page` / `.head` / `.nav-btn` 只剩「听歌排行」在用 ——
    那一个仍然是页面。
    **词汇表里只放词汇**：QQ 表单和同步行在 `Account.module.scss`，文件夹选择在 `Profile`，
    排行列表在 `StatsPage` —— 一个「什么都有点」的共享文件就不再是词汇表，而是没人敢改的地方。
  - **`account` / `profile` 不再是 tab，是面板**（`MusicApp` 的 `tab` 只剩 `list` / `stats`）。
    `useSheet()` 是外壳里那个三份共用的开合状态；`useDataSync({ active: account.open })`
    跟着面板走。迷你条仍然只在 `list` 上出现 —— 账号面板是设置，不是播放界面。
  ...
  - **卡片什么时候出现**：`showData = Boolean(qq) || (ready && (pending > 0 || notice))`。
    `ready` 隔开「还没数过」和「数过了是 0」—— 两个 tab 页都常驻挂载、面板按需挂载，
...
- **「音乐库」抽屉页（`h5/Profile.js`）是「这里有哪些歌」那一半**，和「账号」的分工没变：
  **「音乐库」管「这里有哪些歌」，「账号」管「听歌的是谁」**。它从列表的 ⋮ 抽屉升起，
  ...
  - **`谷歌云盘链接` 又回到了 ⋮ 抽屉**。上次它从抽屉搬到这里，理由是这个面板管曲库、
    而 `音乐库` 那两行只在连上云盘后才出现；现在它回去，用的是同一个论证的另一半：
    **连接是「这个曲库从哪来」的菜单动作，而抽屉就是那个菜单**。留在这里的是访客想
    *看* 的东西 —— 曲库名字、歌数、文件夹选择器。
  ...
  - 卡片上的徽标是**蓝色**，和「账号」页 QQ 徽标的**绿色**是两对：两个不同的问题，
    有意用两种颜色（见上面那条）。
  - **没连云盘时，这个面板很空，这是对的**：一张卡（哪个曲库、多少首、无需授权）加一段
    说明。它不摆一个「连接自己的云盘」按钮 —— 那个动作在 ⋮ 抽屉里，而
    **列表空的时候那个提示按钮会直接开云盘 sheet**（`TrackList` 的 `onOpenDrive`）：
    一个空的曲库只有一步有用的下一步，不该让访客先在一串选项里猜。面板里再说一遍
    等于承认抽屉那个入口不好找。
...
它是**两个 tab 页之一**（`tab === 'stats'`；另一个是 `list`），两个都常驻挂载 ——
`usePlayStats({ active: tab === 'stats' })` 里的 `active` 是「在看这一页」，
不是「挂载了」，否则每次打开应用都要拉一次排行。**「账号」和「音乐库」不是 tab，
是两个抽屉页**，所以这个页面的「账号」按钮是**升起**那个面板，不是切 tab ——
面板放下时你回到这里，而不是回到列表。

**两份数据来自两个查询**（全部 / 最近 7 天是同一个 SQL 换 `WHERE`），
所以列表上那行汇总必须跟着当前 tab 走，不能用同一个 total 写两遍。
列表为空时**不画汇总**（「共播放 0 次」和「还没有播放记录」是同一句话说两遍）。
没绑号时那一页画的是**一句解释 + 一个「去确认 QQ 号」按钮**（`primary-btn`），
不是空列表。
...
- `shell` —— **手机端的外壳结构**：⋮ 还在、抽屉里是 `音乐库 / 谷歌云盘链接 / 缓存管理`、
  `音乐库` 和 `账号` 是**抽屉页**（`[role="dialog"]`，**没有 `<header>`**、有「收起」按钮）、
  抽屉在被打开另一个面板时自己退场；然后是**没号码的喜欢只在本机**（落在 `guest` 桶、
  outbox 是空的），**确认号码之后它跟着号码走** —— 这一条读的是「数据同步」那一行
  自己说的话（`本地还有 N 条数据没上传`），因为那正是访客能看到的证据。
