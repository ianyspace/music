...
| `components/Music/h5/` | 手机端：`MusicApp` 外壳 + 列表/播放页/迷你条/我的/账号/听歌排行 + 三个底部面板 |
...
- **「喜欢」搬去了 D1，而且是按 QQ 分的**（`components/Music/likes.js`，见「我喜欢（D1）」一节）。
  ...
  **没有绑定 QQ 就没有喜欢**：`toggleLike` 会 `setNotice('先绑定 QQ 号，才能喜欢歌曲')` 并返回，
  列表顶栏的 ❤ 过滤器、播放页那颗心、行抽屉里那一项在没绑号时都不渲染。
  旧的那份本机 `liked` 不会被丢掉 —— `migrateLegacyLikes` 在绑号时把它塞进 outbox
  再删掉旧 key（见「我喜欢（D1）」）。
...
- **手机端顶部栏里是应用图标，在最右边，不是曲库名也不是访客头像**（`h5/TrackList.js` 的
  `<header>`）。品牌名和标题被拿掉了，位置给了一个 **`public/icon-192.png` 的 `<img>`**
  （`.mark` / `.mark-img`），它同时是**进入「账号」页的按钮**：
  - **它在最右边，因为它通向「设置」而不是「内容」**：列表的左边缘是内容开始的地方，
    一个要离开列表的控件不该是眼睛落下的第一件事。它曾经在左边（那里原来是曲库名）。
  - **`margin-left: auto` 是承重的**，不是顺手写的：这一行的另一个子节点是 `sr-only` 的
    `<h1>`，而它是 `position: absolute`，**根本不是 flex item**。`space-between` 遇到
    只有一个在流内的子节点时会把那个子节点放在**开头** —— 去掉这个 auto margin，
    整组按钮（搜索 / ❤ / 图标）会在图标挪出首位的那一刻跳到最左边。
  ...
  - **它仍然是按钮，因为背后终于有东西了**：点它进「账号」页（`h5/Account.js`）。
...
- **列表的 ⋮ 抽屉已经整个删掉**（`h5/MusicApp.js`）。它一度只剩一个条目
  （谷歌云盘链接），而**只有一个条目的菜单比没有菜单更糟**：多一次点击、多一次判断，
  换来的东西本来可以是一行。条目本身搬去了「我的」（见下一条），⋮ 和抽屉一起消失，
  `menuOpen` / `menuClosing` / `closeMenu` / `openMenu` / Escape 监听都跟着走了。
  **行抽屉（每首歌的 ⋮）不是一回事，它留着**：它是逐首的、不止一个条目，
  而且必须挂在外壳这一层（列表列在 `transform` 祖先下，`position: fixed` 会被收进去）。
- **「账号」页（`h5/Account.js`）是访客自己的页面**，从列表**右上角的**应用图标进入，和「我的」
  （`h5/Profile.js`）分工明确：**「我的」管「这里有哪些歌」，「账号」管「听歌的是谁」**。
  它装了原来列表 ⋮ 抽屉里的 `缓存管理` 和 `切换外观`，加上 QQ 号确认和「我的数据」
  （`听歌排行` 入口 + `数据同步`）。**抽屉和它的 ⋮ 都已经删掉**，原来唯一的条目
  `谷歌云盘链接` 搬去了「我的」—— 那是「这里有哪些歌」那一半。
  ...
  - **文案统一用「确认」，不用「绑定」**，因为那才是访客做的动作（按钮也从 `确定` 改成 `确认`）。
    没号码时身份卡的名字是 `访客`（不是「还没有确认 QQ 号」）—— 状态已经在旁边的徽标里了，
    名字那一行再重复一遍只会把号码挤到省略号。
  ...
  - **QQ 号做三件事：换头像、当播放次数的归属、当「我喜欢」的归属**（`shared.js` 的
    ...
    因为拒绝这种输入教不了任何人任何事。
  - **号码本身归 `usePlayer`，不是归这个页面**（`usePlayer` 的 `qq` / `saveQq`）。它以前是
  ...
  - **页面外壳已经抽成 `settingsPage.module.scss`**（page / head / title / nav-btn /
    refresh-btn / primary-btn / group / row / hint / footnote），由「我的」「账号」「听歌排行」
    三个页面 `composes:`。这三个形状曾经**故意**在 Profile / Account 里各抄一份，
    两边的注释都写着「出现第三个设置风格页面时就是抽取的时机」——**「听歌排行」就是那一刻**，
    于是把「我的」一起折了进来，而不是留一个页面用旧副本。
    **壳里只放壳**：QQ 表单和同步行在 `Account.module.scss`，文件夹选择在 `Profile`，
    排行列表在 `StatsPage` —— 一个「什么都有点」的共享文件就不再是壳，而是没人敢改的地方。
  - **它的 tab 名是 `account`**（`MusicApp` 的 `tab`），迷你条仍然只在 `list` 上出现 ——
    账号页是设置，不是播放界面。
  ...
  - **卡片什么时候出现**：`showData = Boolean(qq) || (ready && (pending > 0 || notice))`。
    `ready` 隔开「还没数过」和「数过了是 0」—— 四个 tab 页都常驻挂载，
    所以这个 hook 在没人看的时候根本不该读 localStorage，更不能在没数过时声称「已全部上传」。
    没绑号时**不画「听歌排行」那一行**（排行是按 QQ 分的，没号就没有东西可开），
    但「数据同步」那一行可能在 —— 见上面「清除号码」那条。
- **「我的」页（`h5/Profile.js`）现在也是连接云盘的地方**，和「账号」页的分工没变：
  **「我的」管「这里有哪些歌」，「账号」管「听歌的是谁」**。`谷歌云盘链接` 从列表 ⋮ 抽屉
  搬过来，落在它本来就应该在的地方 —— **`音乐库` 那两行（文件夹 / 浏览歌曲）只在连上
  云盘之后才出现**，而让它们出现的那个开关，此前长在另一个页面的菜单里。
  - **连接流程本身还是那个 sheet**（`DriveSheet`），没有搬进页面：带客户端 ID 输入框的
    多步授权不是一行设置，而且 sheet 不属于任何一个 tab（它是盖在整列上的整屏面板）。
    「我的」上是一行入口：没连上时叫 `连接自己的云盘`，连上了叫 `云盘设置`（断开连接
    在那个 sheet 里，因为它是「再点一次确认」的两段式，不该离文件夹选择器一个手滑那么近）。
  - **顶部那张卡是照着云盘 sheet 的卡做的**：同一个蓝色圆角图标、同一个 `已连接` /
    `未连接` 徽标。**两张卡回答的是同一个问题（我现在在哪个曲库），就该用同一种方式回答。**
    徽标是这次新加的：在那之前，唯一能分辨「现在是公共曲库还是我的云盘」的办法是
    读标题、并且知道默认那个曲库叫什么名字。
  - **它不再包在 `.group` 里**：它现在是页面自己的主体，而不是设置卡片里的一行，
    卡片套卡片读起来像画错了。所以 `.account` 自己带 `margin-bottom` / 圆角 / `--field` 底。
  - 卡片上的徽标是**蓝色**，和「账号」页 QQ 徽标的**绿色**是两对：两个不同的问题，
    有意用两种颜色（见上面那条）。
  - **头部那两个 `nav-btn` 是承重的，不是装饰**：在这次改动之前，「我的」全应用**只有一个
    入口** —— 歌曲列表的「曲库里还没有歌曲」提示。也就是说曲库正常有歌的时候，
    这个页面根本打不开。把连接云盘搬进去，等于把入口藏进一个没人到得了的页面。
    现在两个设置风格页面互相指向对方（「我的」头部有 `账号`，「账号」头部有 `我的`），
    从任意一个到「歌曲」都是一跳。
...
| `components/Music/h5/usePlayStats.js` | 「听歌排行」页的状态（加载 / 错误 / 刷新） |
...
**排行页（`h5/StatsPage.js`）从「账号」页里搬出来单独成页。** 它曾经是账号页底部一张卡片，
搬家的两个理由：五十首歌的列表是一个**目的地**而不是一张设置卡；而且它只对绑了号的访客
有意义，做成页面就可以干脆不存在，而不是变成一张必须解释自己为什么是空的卡片。
它是**第四个 tab 页**（`tab === 'stats'`），和另外三个一样常驻挂载 ——
`usePlayStats({ active: tab === 'stats' })` 里的 `active` 是「在看这一页」，
不是「挂载了」，否则每次打开应用都要拉一次排行。

**两份数据来自两个查询**（全部 / 最近 7 天是同一个 SQL 换 `WHERE`），
所以列表上那行汇总必须跟着当前 tab 走，不能用同一个 total 写两遍。
列表为空时**不画汇总**（「共播放 0 次」和「还没有播放记录」是同一句话说两遍）。
没绑号时那一页画的是**一句解释 + 一个「去绑定 QQ 号」按钮**（`primary-btn`），
不是空列表。

**「同步」按钮现在在「账号」页，而且管两支队列**（`useDataSync`，见「账号」页那一条）。
...
2. **拉取失败绝不清镜像。** 和列表缓存（`readListCache`）同一条规矩：断网意味着
   「继续显示我们原来有的」，不是「你没有喜欢」。
3. **镜像是按 QQ 分键的**（`music:likes:<qq>`）。换号绝不能显示上一个号的喜欢 ——
   和列表缓存按来源分键同一个理由。
...
第一次绘制就是对的。`readLikedKeys(qq)` 因此是同步的，没绑号返回 `[]` —— 那也是诚实的答案。
...
**没绑 QQ 就没有喜欢**：`toggleLike` 会 `setNotice('先绑定 QQ 号，才能喜欢歌曲')` 并返回，
列表顶栏的 ❤ 过滤器、播放页那颗心、行抽屉里那一项都不渲染。旧的那份本机 `liked`
由 `migrateLegacyLikes(qq)` 在绑号时塞进 outbox（**再删掉旧 key**，第二次跑不能重复入队），
所以升级不会看起来像「更新吃掉了我的喜欢」；它走的是同一条 outbox，离线也能用。
旧的键是 `<source>:<id>` 形式，只有 `cloud:` 的被留下 —— 和 `toggleLike` 拒绝 Drive 同一个理由。
...
h5 另外挂了一段自己的：`like`（21 项）—— 从行抽屉喜欢 → 只看喜欢只剩那一首 →
**`Page.reload` 之后还在** → 播放页那颗心点亮并能取消 → 空的我喜欢有自己的文案 →
Drive 曲库下整套入口消失。**这是唯一会重载页面的一段，所以它放在最后**，
否则它会把前面所有检查测量的页面换掉。

> **这段现在是坏的，别信它跑出来的结果。** 喜欢搬去 D1 之后，整条路径要先绑一个 QQ 号、
> 而且要有真的 Worker 在跑 —— 它既没绑号也没打桩 `/likes`，所以第一步（找到 ❤ 过滤器）
> 就会落空。修它需要三件事：注入一个 QQ 号、把 `GET/POST /likes` 在网络层打桩
> （和 `stubDriveFiles` 同一套 CDP `Fetch` 手法）、把「reload 之后还在」改成
> 「reload 之后镜像还在」。**修之前请把它标成已知失败，而不是把断言删掉** ——
> 它原来兜的正是「只改一套布局」那类缝。
