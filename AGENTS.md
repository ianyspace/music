...
- **空列表的文案只有一处决策**（`shared.js` 的 `emptyListMessage`），五个分支按顺序判断：
  还在加载 → 搜了没搜到 → 「只看喜欢」开着 → `libraryCount > 0` 说明歌都被移进「不喜欢」了 → 曲库真的空。
  三个容易搞错的地方：
...
- **列表偏好有三个，形状一样但不是一类**（`shared.js` 的 `DISLIKED_KEY` / `ORDER_KEY` / `LIKED_KEY`）：
  `disliked` 是排除表、`order` 是置顶排序、`liked` 是保留表。都是 `<source>:<id>` 的 JSON 数组，
  都走 `readKeyList` / `writeKeyList`，都在 `localStorage`。
  三个一起进 `visibleTracks`，顺序**不是随便排的**：`applyListPrefs`（排除 + 置顶）→ 只看喜欢 → 搜索。
  不喜欢必须**先于**喜欢，一个既喜欢又被移入不喜欢的歌要保持隐藏，否则别处的按钮就把访客
  刚才那一下撤销了；喜欢又必须在搜索**之前**，因为搜索是对「已决定显示什么」的再收窄。
...
- **置顶是可逆的，而「取消置顶 = 恢复默认排序」不需要任何新数据**。`order` 是**排名**而不是全排列，
  只存访客置顶过的 key；没有 entry 的歌在 `applyListPrefs` 的稳定部分排序里就落在源顺序 ——
...
h5 上还挂了一段**「我喜欢」的完整回归**（`like` 那一块，27 项），因为它有必须活过刷新的状态：
从行抽屉喜欢 → 只看喜欢只剩那一首 → **`Page.reload` 之后还在** → 播放页那颗心点亮并能取消 →
空的我喜欢有自己的文案 → 喜欢和移入不喜欢冲突时隐藏赢 → Drive 曲库下整套入口消失。
...
- **同理，别用子串判断文案。** `'移入不喜欢'.includes('喜欢')` 是 **true** —— 拿
  `includes('喜欢')` 去查抽屉里有没有喜欢项，会在一个**正确**的抽屉上失败，却放过一个
  碰巧多出喜欢项的抽屉。标题一律取回来当数组、整体比较。
