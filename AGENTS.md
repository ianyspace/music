...
- **手机端顶部栏里是应用图标，不是曲库名、也不是访客头像**（`h5/TrackList.js` 的 `<header>`）。
  品牌名和标题被拿掉了，位置给了一个 **`public/icon-192.png` 的 `<img>`**（`.mark` / `.mark-img`），
  它同时是**进入「账号」页的按钮**：
...
- **「账号」页（`h5/Account.js`）是访客自己的页面**，从列表左上角的**应用图标**进入，和「我的」
  （`h5/Profile.js`）分工明确：**「我的」管「这里有哪些歌」，「账号」管「听歌的是谁」**。
  它装了原来列表 ⋮ 抽屉里的 `缓存管理` 和 `切换外观`，加上 QQ 号绑定和「我的数据」
  （`听歌排行` 入口 + `数据同步`）。抽屉里**只留** `谷歌云盘链接`：⋮ 长在曲库上，
  而个人设置和曲库不是一类东西 —— 一个抽屉里混着两类动作，访客就得先想「我要找的那个大概在哪一类里」。
